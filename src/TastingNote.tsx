...
type Category = "전체" | "칵테일" | "위스키" | "진, 럼, 데낄라" | "와인" | "기타"; // 카테고리 타입 정의
const TasteNote: React.FC = () => {
    ...
    const categorizedDrinks = {
        전체: drinks,
        칵테일: drinks.filter((drink) => drink.category === "cocktail"),
        위스키: drinks.filter((drink) => drink.category === "whiskey"),
        "진, 럼, 데낄라": drinks.filter((drink) => ["gin&rum&tequila"].includes(drink.category)),
        와인: drinks.filter((drink) => drink.category === "wine"),
        기타: drinks.filter((drink) => drink.category === "other"),
    };

    // 선택된 카테고리에서 음료를 필터링
    const filteredDrinks = categorizedDrinks[filter];
    ...
    useEffect(() => {
        console.log("현재 필터링된 음료들:", filteredDrinks);
    }, [filteredDrinks]);
    ...
    const handleEditDrink = async (updatedDrink: Drink) => {
        ...
        try {
            // 만약 noteId가 있으면 이전 noteId를 가져와서 서버에 수정 요청
            let noteIdToUpdate = updatedDrink.noteId;
            if (updatedDrink.noteId) {
                noteIdToUpdate = await getPreviousNoteId(updatedDrink.noteId);
            }
    
            // 서버에 PATCH 요청
            const response = await fetch(
                `http://54.180.45.230:3000/api/v1/users/tasting-note/${noteIdToUpdate}?type=${updatedDrink.category}`,
                {
                    method: "PATCH",
                    headers: {
                        "Content-Type": "application/json",
    ...
    useEffect(() => {
        console.log("현재 카테고리:", filter);
        console.log("필터링된 음료들:", categorizedDrinks[filter]);
        setDrinks(categorizedDrinks[filter]);
    }, [filter]);
