...
const LoginPage: React.FC = () => {
  ...
  const handleLogin = async () => {
    if (!email || !password) {
      setErrorMessage("이메일 또는 비밀번호를 입력하지 않았습니다. 다시 확인해주세요.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 401) {
          setErrorMessage("이메일 또는 비밀번호가 맞지 않습니다. 다시 확인해주세요.");
        } else if (response.status >= 500) {
          setErrorMessage("서버 연동에 실패했습니다.");
        } else {
  ...
  return (
    <>
      ...
      <div className="login-container">
              ...
              <input
                className={`${
                    errorMessage === "이메일 또는 비밀번호가 맞지 않습니다. 다시 확인해주세요."
                      ? "error-password"
                      : errorMessage === "서버 연동에 실패했습니다."
                      ? "error-input"
                      : errorMessage === "예기치 않은 오류가 발생했습니다. 다시 로그인해보세요."
                      ? "error-input"
                      : errorMessage === "이메일 또는 비밀번호를 입력하지 않았습니다. 다시 확인해주세요."
                      ? "error-input"
                      : ""  
                  }`}
        ...
        {/* 오류 메시지 */}
        {errorMessage && <p className="loginerror-message">{errorMessage}</p>}
        ...
        <button
          ...
          onClick={() =>
            (window.location.href = "https://your-kakao-auth.com")
          }
        >
        <svg className="kakaoicon" xmlns="http://www.w3.org/2000/svg" width="36" height="35" viewBox="0 0 36 35" fill="none">
             <ellipse cx="18" cy="14" rx="18" ry="14" fill="black"/>
            <path d="M6 34L8 25L17 27L6 34Z" fill="black" stroke="black" stroke-linejoin="round"/>
        </svg>
          Login With KaKao
        </button>
        <button
