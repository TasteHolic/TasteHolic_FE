...
const TasteNote: React.FC = () => {
  ...
  const [editTarget, setEditTarget] = useState<Drink | null>(null); // 수정할 술 정보 상태 추가
    ...
    <>
      ...
      <div className={`taste-note-container ${deleteTarget ? "blurred" : ""}`}>
        <header>
          ...
          <div className="taste-note-dropdown">
            ...
            >
              ...
              {!isOpen && (
                <img src="/image/arrow-down.png" alt="드롭다운 화살표" />
              )}
            </button>
            {isOpen && (
              <ul
                className={`taste-note-dropdown-menu ${isOpen ? "open" : ""}`}
              >
                {["recent", "rating"]
        ...
        <div className="taste-note-drink-list">
          {sortedDrinks.length > 0 && (
            ...
            >
              <img src="/image/addbutton.png" alt="추가버튼"></img>
            </button>
          )}
          {sortedDrinks.length > 0 ? (
              ...
              <div key={drink.id} className="taste-note-drink-item">
                <div className="taste-note-drink-hover-container">
                  ...
                  <button
                    ...
                    onClick={() => setDeleteTarget(drink)}
                  >
                    <img src="/image/trash.png" alt="삭제버튼"></img>
                  </button>
                </div>
              </div>
            ))
            ...
            <div className="taste-note-empty-container">
              <button
                className="taste-note-add-drink center"
                onClick={() => setIsModalOpen(true)}
              >
                <img src="/image/addbutton.png" alt="추가버튼"></img>
              </button>
              <div className="taste-note-empty-message">텅 비었네요!</div>
              <div>오늘의 특별한 한 잔을 기록하러 가볼까요?</div>
            </div>
          )}
        </div>
      </div>
      ...
      {deleteTarget && (
        <div className="taste-note-delete-modal">
          <p>삭제하시겠습니까?</p>
          <button onClick={handleDeleteDrink}>삭제</button>
