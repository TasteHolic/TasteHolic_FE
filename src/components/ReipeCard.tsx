...
const getAlcoholLabel = (abv: number): string => {
  if (abv === 0) return "Nonalcoholic";
  if (abv > 0 && abv <= 15) return "Light";
  if (abv >= 16 && abv <= 30) return "Medium";
  return "Strong";
};
    ...
    const fetchRecipeDetails = async () => {
      try {
        ...
        if (response.data.resultType === "SUCCESS") {
          ...
          setName(recipe.nameEng || recipe.name);
          setImage(recipe.imageUrl || "/image/image 92.png");

          setKeyWords([
            { label: "칵테일", type: "variety" },
            ...recipe.tastes.map((taste: any) => ({ label: taste, type: "flavor" })),
            ...recipe.aromas.map((aroma: any) => ({ label: aroma, type: "aroma" })),
            { label: getAlcoholLabel(recipe.abv), type: "alcoholPer" },
          ]);
        } else {
          throw new Error("레시피 데이터를 가져오지 못했습니다.");
        }
      } catch (error) {
